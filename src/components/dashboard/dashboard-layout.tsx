...
export function DashboardLayout({ children, title, eyebrow }: { children: ReactNode; title: string; eyebrow?: string }) {
  ...
  return <div className="h-screen overflow-hidden p-3 sm:p-5 lg:p-7"><div className="mx-auto flex h-full max-w-[1500px] gap-4">
    ...
    <section className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-[2rem] border border-base-300/60 bg-base-100/80 shadow-xl shadow-base-content/5 backdrop-blur-sm"><header className="flex min-h-[92px] shrink-0 items-center justify-between gap-4 border-b border-base-200/80 px-5 sm:px-8"><div className="flex items-center gap-3"><button className="btn btn-ghost btn-square lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Ouvrir le menu"><Menu className="size-5" /></button><div><div className="text-xs font-extrabold uppercase tracking-[0.18em] text-primary">{eyebrow ?? 'Espace personnel'}</div><h1 className="font-display text-2xl font-bold tracking-tight sm:text-3xl">{title}</h1></div></div><div className="flex items-center gap-2 sm:gap-4"><button className="btn btn-ghost btn-circle relative"><Bell className="size-5" /><span className="absolute right-2 top-2 size-2 rounded-full bg-secondary ring-2 ring-base-100" /></button><div className="hidden h-8 w-px bg-base-300 sm:block" /><div className="dropdown dropdown-end"><button className="flex items-center gap-2 rounded-full p-1 pr-2 hover:bg-base-200"><div className="avatar placeholder"><div className="w-9 rounded-full bg-primary text-xs font-extrabold text-primary-content">DR</div></div><span className="hidden text-sm font-bold sm:inline">Dieuveil</span><ChevronDown className="size-4 text-base-content/50" /></button><ul className="menu dropdown-content z-[1] mt-3 w-48 rounded-2xl border border-base-200 bg-base-100 p-2 shadow-xl"><li><Link to="/dashboard/settings">Mon profil</Link></li><li><button>Se déconnecter</button></li></ul></div></div></header><main className="min-h-0 flex-1 overflow-y-auto p-5 sm:p-8">{children}</main></section>
  </div></div>
}
